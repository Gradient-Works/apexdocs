...
export default class GWGitbookDocsProcessor extends DocsProcessor {
  ...
  process(classModel: ClassModel, outputDir: string) {
    const level = 1;
    const gen = this.getGenerator(classModel.getClassName());
    if (classModel.getClassName().endsWith('Action')) {
      this.generateActionDocs(gen, classModel, 1);
    } else {
      this.generateModelDocs(gen, classModel, 1);
    }
  ...
  public onAfterProcess(classes: ClassModel[], outputDir: string) {
    const modelsToc = new Map<string, string>();
    const actionsToc = new Map<string, Map<string, string>>();

    const abkDir = path.join(outputDir, 'generated');
    const actionsDir = path.join(abkDir, 'actions');
    const modelsDir = path.join(abkDir, 'models');
    this.classes.forEach(classModel => {
      const className = classModel.getClassName();
      const title = this.getClassTitle(classModel);
      const generator = this.getGenerator(className);
      let fileDir = modelsDir;
      if (this.isAction(classModel)) {
        ...
        let categoryFSName = category.toLowerCase().replace(/\s+/, '_');
        fileDir = path.join(actionsDir, categoryFSName);
        let catToc = actionsToc.get(category);
        if (catToc == null) {
          catToc = new Map<string, string>();
          actionsToc.set(category, catToc);
        }
        catToc.set(actionTitle, `generated/actions/${categoryFSName}/${className}.md`);
      } else {
        modelsToc.set(title, `generated/models/${className}.md`);
      }

      if (!fs.existsSync(fileDir)) {
        fs.mkdirSync(fileDir, { recursive: true });
      }
      const filePath = path.join(fileDir, `${className}.md`);
      fs.writeFile(filePath, generator.contents, 'utf8', () => {
        // tslint:disable-next-line:no-console
        console.log(`${className} data written to ${filePath}`);
      });
    });

    // Output the summary - this is messy but whatever
    const summary = this.getGenerator('summary');
    summary.addText('* [Actions](abk/actions/index.md)');
    [...actionsToc.entries()]
      .sort(([t1, p1], [t2, p2]) => t1.localeCompare(t2))
      .forEach(([category, catToc]) => {
        summary.addText(`  * [${category}](abk/actions/${category.toLowerCase().replace(/\s+/, '_')}/index.md)`);
        [...catToc.entries()]
          .sort(([t1, p1], [t2, p2]) => t1.localeCompare(t2))
          .forEach(([title, path]) => {
            summary.addText(`    * [${title}](${path})`);
          });
      });
    summary.addText('* [Models](abk/models/index.md)');
    [...modelsToc.entries()]
      .sort(([t1, p1], [t2, p2]) => t1.localeCompare(t2))
      .forEach(([title, path]) => {
        summary.addText(`  * [${title}](${path})`);
      });
    const filePath = path.join(abkDir, 'SUMMARY.md');
    fs.writeFile(filePath, summary.contents, 'utf8', () => {
      // tslint:disable-next-line:no-console
      console.log(`SUMMARY.md written to ${filePath}`);
    });
  }
  ...
  private generateActionDocs(generator: MarkdownHelper, classModel: ClassModel, level: number) {
    ...
    generator.addText(
      `<figure><img src="../.gitbook/assets/${classModel.getClassName()}.png" alt="${title}"></figure>`,
    );
  ...
  private classLink(type: string, referencedClass?: ClassModel) {
    ...
    if (referencedClass) {
      ...
      if (this.isAction(referencedClass)) {
        ...
        let categoryFSName = category.toLowerCase().replace(/\s+/, '_');
        path = `generated/actions/${categoryFSName}`;
      } else {
        path = `generated/models`;
      }
      return `[${title}](${path}/${referencedClass.getClassName()}.md)`;
    } else {
      return type;
    }
  }
  ...
  private isAction(classModel: ClassModel): boolean {
    return classModel.getClassName().endsWith('Action');
  }
}
